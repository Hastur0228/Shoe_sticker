...
def process_directory(input_dir, output_dir, num_points=2048, uniform_sampling=True):
    """
    处理目录中的所有STL文件
    ...
    Args:
        ...
        uniform_sampling (bool): 是否使用均匀采样
    """
    # 创建输出目录
    ...
    for stl_file in stl_files:
        ...
        # 处理文件
        if stl_to_pointcloud(stl_file, output_file, num_points, uniform_sampling):
            success_count += 1
...
def main():
    ...
    if args.subdir:
        ...
        if os.path.exists(input_subdir):
            print(f"\n处理子目录: {args.subdir}")
            process_directory(input_subdir, output_subdir, args.num_points, uniform_sampling)
        else:
            print(f"子目录不存在: {input_subdir}")
    elif args.main_dir_only:
        # 只处理主目录
        process_directory(args.input_dir, args.output_dir, args.num_points, uniform_sampling)
    else:
        # 默认处理所有子目录
